...
def rotateScreen():
# Untuk mode 3D : Memutar camera dengan directional buttons
...
# Menampilkan animasi translasi semua titik verticies ke titik nextvert ekuivalennya
    ...
    for n in range(100):
        for i in range(int(verticies.size/3)):
            ...
            temp = translate3D(np.array([anivert[i]]), dx/100, dy/100, dz/100)
            anivert[i] = temp[0]
        ...
        Sumbu3D()
        pygame.display.flip()
        pygame.time.wait(round(3000/100))
    return
...
# Menampilkan animasi translasi semua titik verticies ke titik nextvert ekuivalennya
	...
	for n in range(100):
		for i in range(int(verticies.size/2)):
			dx = nextvert[i][0] - verticies[i][0]
			dy = nextvert[i][1] - verticies[i][1]
			temp = translate2D(np.array([anivert[i]]), dx/100, dy/100)
			anivert[i] = temp[0]
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
		rotateScreen()
...
# Thread untuk mendaptkan command dari user sambil merender object di window pygame
    def __init__(self):
...
def inputVerticies():
# Menerima input titik-titik dari user untuk membangun shape 2D (Titik pertama akan terhubung dengan titik dua dst)
    global N
    ...
    while N < 3:
        ...
        N = int(input("Masukan Jumlah Titik : "))
    for i in range(N):
        print("Masukan titik ke-" + str(i+1) + " (format : x y): ")
        vertex = (input().split(' '))
        x = int(vertex[0])
        y = int(vertex[1])
        vertex[0] = x
...
def main3D():
# Program utama 3D
    # Definisi kubus secara hard-code
    verticies = np.array([
        [50, -50, -50],
        [50, 50, -50],
        [-50, 50, -50],
    ...
    while cmd != 'exit':
        ...
        if commandexist:
            ...
            if arg[0] == 'translate':
                ...
                for n in range(100):
                    nextvert = translate3D(nextvert, dx/100, dy/100, dz/100)
                    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
                    ShadowCube(verticies, edges, surfaces)
                ...
                nextvert = np.copy(verticies)
                for n in range(100):
                    nextvert = dilate3D(nextvert, k**(1./100))
                    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
                    ShadowCube(verticies, edges, surfaces)
                    Cube(nextvert, edges, surfaces, colors)
                    Sumbu3D()
                ...
                nextvert = np.copy(verticies)
                for n in range(100):
                      nextvert = rotate3D(nextvert, a, b, c, degree/100)
                      glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
                      ShadowCube(verticies, edges, surfaces)
                      Cube(nextvert, edges, surfaces, colors)
                      Sumbu3D()
                ...
                nextvert = np.copy(verticies)
                for n in range(100):
                    nextvert = shear3D(nextvert,param1,param2,(k/100))
                    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)
                    ShadowCube(verticies,edges,surfaces)
                    Cube(nextvert,edges,surfaces,colors)
                    Sumbu3D()
                ...
                nextvert = np.copy(verticies)
                for n in range(100):
                    nextvert = stretch3D(nextvert,param,(k**(1./100)))
                    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)
                    ShadowCube(verticies,edges,surfaces)
                    Cube(nextvert,edges,surfaces,colors)
                    Sumbu3D()
            ...
            # Multiple akan menampung N command dalam multicmd dalam thread getCommand dan menjalankannya satu demi satu
                for cmd in multicmd:
                    arg = cmd.split(' ')
                    if arg[0] == 'translate':
                        dx = float(arg[1])
    ...
    while cmd != 'exit':
        ...
        if commandexist:
            ...
            if arg[0] == 'translate':
                ...
                nextvert = np.copy(verticies)
                for n in range(100):
                    nextvert = translate2D(nextvert,dx/100,dy/100)
                    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)
                    ShadowShape(verticies)
                    Shape(nextvert)
                    Sumbu2D()
                ...
                nextvert = np.copy(verticies)
                for n in range(100):
                    nextvert = dilate2D(nextvert,k**(1./100))
                    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)
                    ShadowShape(verticies)
                    Shape(nextvert)
                    Sumbu2D()
                ...
                nextvert = np.copy(verticies)
                for n in range(100):
                    nextvert = rotate2D(nextvert,(degree/100), x, y)
                    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)
                    ShadowShape(verticies)
                    Shape(nextvert)
                    Sumbu2D()
                ...
                nextvert = np.copy(verticies)
                for n in range(100):
                    nextvert = shear2D(nextvert,param,(k/100))
                    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)
                    ShadowShape(verticies)
                    Shape(nextvert)
                    Sumbu2D()
                ...
                nextvert = np.copy(verticies)
                for n in range(100):
                    nextvert = stretch2D(nextvert,param,(k**(1./100)))
                    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)
                    ShadowShape(verticies)
                    Shape(nextvert)
                    Sumbu2D()
